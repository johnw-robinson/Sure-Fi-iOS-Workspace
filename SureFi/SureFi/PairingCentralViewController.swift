//
//  PairingCentralViewController.swift
//  SureFi
//
...
class PairingCentralViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate {
    ...
    @IBOutlet var centralDescTextView: UITextView!
    @IBOutlet var continueButton: UIButton!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        self.navigationItem.title = "Step 1 - Setup Central"
        
        configureContainerView.isHidden = true
        
        centralImageView.layer.borderColor = UIColor.gray.cgColor
        centralImageView.layer.borderWidth = 0.5
        centralImageView.layer.cornerRadius = 5
        centralImageView.clipsToBounds = true
        
        takeImageButton.layer.cornerRadius = 5
        takeImageButton.clipsToBounds = true
        
        centralDescTextView.layer.borderColor = UIColor.gray.cgColor
        centralDescTextView.layer.borderWidth = 0.5
        
        centralDescTextView.text = "Please provide a description...\n\nExample - This Sure-Fi Central Unit is connected to a 4-Door controller from XYZ Company"
        centralDescTextView.textColor = .gray
    ...
    @IBAction func continueButtonPress (sender: UIButton) {
        ...
        pairingTableViewController.centralImage = centralImage
        pairingTableViewController.centralDescription = centralDescTextView.text
        pairingTableViewController.centralDeviceID = selectedDeviceID
        ...
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let controller = storyboard.instantiateViewController(withIdentifier: "PairingRemoteViewController") as! PairingRemoteViewController
    ...
    func textViewDidEndEditing(_ textView: UITextView) {
        ...
        var text = centralDescTextView.text
        if text?.range(of: "Please provide a description...") != nil {
            text = ""
        }
        
        if text != "" && centralImage != nil {
