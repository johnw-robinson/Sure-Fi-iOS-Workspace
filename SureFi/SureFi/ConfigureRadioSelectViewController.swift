//
//  ConfigureRadioSelectViewController.swift
//  SureFi
//
//  Created by John Robinson on 6/16/17.
...
class ConfigureRadioSelectViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {

    var configureRadioTableViewController: ConfigureRadioTableViewController!
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from connection: AVCaptureConnection!) {
        ...
        if metadataObj.type == AVMetadataObjectTypeQRCode {
            ...
            if metadataObj.stringValue != nil {
                ...
                if(configureRadioTableViewController.surefiDevices.count==0) {
                    self.setDeviceFound(found: false, deviceID: "", msg: "", status: "")
                } else {
                    
                    for (key,value) in configureRadioTableViewController.surefiAdvertising {
                        ...
                        if peripheralDeviceID.uppercased().range(of: scannedDeviceID.uppercased()) != nil {
                            ...
                            if deviceStatusString.substring(to: 2) == "01" || deviceStatusString.substring(to: 2) == "02" {
                                self.setDeviceFound(found: true, deviceID: scannedDeviceID ,msg: "", status: "02")
                                return
                            } else {
                                self.setDeviceFound(found: false, deviceID: "", msg: "Device (\(scannedDeviceID!)) Not Sure-Fi Bridge Unit", status: "02")
                                return
                            }
                        } else {
    ...
    func setDeviceFound (found: Bool, deviceID: String, msg: String, status: String) {
        ...
        print("Found:\(found ? "true" : "false") Selected:\(selectedDeviceID) Status:\(selectedDeviceStatus)")
        if deviceFound {
    ...
    @IBAction func confirmButtonPress (sender: UIButton) {
        ...
        configureRadioTableViewController.deviceID = selectedDeviceID
        configureRadioTableViewController.tableView.reloadData()
        configureRadioTableViewController.selectDevicePeripheral()
