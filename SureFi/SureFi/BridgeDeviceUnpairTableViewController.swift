//
//  BridgeDeviceUnpairTableViewController.swift
//  SureFi
//
...
class BridgeDeviceUnpairTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "Unpair Sure-Fi Bridge"
    }
    ...
    @IBAction func continueButtonPress(sender: UIButton) {
        ...
        let alertController = UIAlertController(title: "Continue UnPairing", message: "Are you sure you wish to UnPair the following Sure-Fi Devices:\n\nCentral:\(centralDeviceID)\nfrom\nRemote:\(remoteDeviceID)", preferredStyle: .alert)
        let pairAction = UIAlertAction(title: "UNPAIR", style: .destructive, handler: {
            action in
            self.unpairBridge()
        })
    ...
    func unpairBridge() {
        let ret = sfBridgeController.unpairBridge(centralDeviceID: centralDeviceID, remoteDeviceID: remoteDeviceID)
    }
